...
mensaje3: .asciiz "El número mayor es: "
...
bucle_lectura:
    ...
    addi $t1, $t1, 1  # Contador de números leídos
    j bucle_lectura

fin_lectura:
    # Encontrar el número mayor
    la $t1, numeros   # Cargar la dirección del primer elemento en $t1
    lw $t2, 0($t1)    # Inicializar el mayor con el primer número
    addi $t1, $t1, 4  # Avanzar al segundo número
    li $t3, 1         # Contador de iteraciones

bucle_mayor:
    beq $t3, $t0, fin_mayor  # Si se procesaron todos los números, terminar
    lw $t4, 0($t1)           # Cargar el siguiente número en $t4
    blt $t2, $t4, actualizar # Si el actual es mayor, actualizar
    j siguiente

actualizar:
    move $t2, $t4  # Actualizar el número mayor
